...
函数需返回 {"action": "工具名", "params": {...}} 格式。
在 Unity 端的 ExecuteTool 中添加对应的 case 实现具体操作。
重新运行程序，新工具会自动出现在 LLM 的工具列表中。

修改知识库
project_rules.md 中的内容会被动态注入系统提示，你可以根据项目需求自由增删规范条目。格式为 Markdown，建议保持简洁。
